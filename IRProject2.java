...
public class IRProject2 {
	...
	public static void main(String args[]) throws IOException
	{
		...
		//Path path1 = fs.getPath("C:/Users/Aslam/Desktop/index"); //Reading the path of the index as a predefined path
		Path path1 = fs.getPath(args[0]); //Reading the path of the index as a command line argument
		Map<String,LinkedList<Integer>> dictionary=new HashMap<>();
		IndexReader reader = DirectoryReader.open(FSDirectory.open(path1));
		
		File index1=new File("index.txt");
        PrintStream FileWriter=new PrintStream(index1,"UTF-8");
		...
		while(itr.hasNext())
		{
			...
			while(termsEnum.next()!=null)
			{
				...
				{ 
					id=postingEnum.docID(); //Obtaining the docId for the term
		...
		try
		{
			Reader readerOfFile=new InputStreamReader(new FileInputStream(args[2]),"utf-8"); //reading the input file as command line argument
			//Reader readerOfFile=new InputStreamReader(new FileInputStream("C:/Users/Aslam/Desktop/input.txt"),"utf-8"); //reading the input file as predefined path
			BufferedReader inputFile=new BufferedReader(readerOfFile);
			PrintWriter writer=new PrintWriter(new OutputStreamWriter(new FileOutputStream(args[1]),StandardCharsets.UTF_8),true); //reading the output File as argument
			//PrintWriter writer=new PrintWriter(new OutputStreamWriter(new FileOutputStream("C:/Users/Aslam/Desktop/output.txt"),StandardCharsets.UTF_8),true); //reading the output File as predefined path
			String query=null;
			while((query=inputFile.readLine())!=null) //Reading the data from the file until End Of Function
			{
				...
				{ 	
					...
					if(dictionary.containsKey(queryTerm))
					{
						...
						{
							writer.append(getPostingsList.get(postingListIndex++)+" "); //Appending each docId for the term to an output file
						}
						writer.append("\r\n");
					}
